...
// Desenha um quadrado no display com tamanho e posição definidos
void ssd1306_draw_square(ssd1306_t *ssd, uint8_t x, uint8_t y, uint8_t size) {
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            ssd1306_set_pixel(ssd->ram_buffer, x + i, y + j, true);
        }
    }
